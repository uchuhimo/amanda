...
struct HookOpRegistrationListener : public c10::OpRegistrationListener {
  ...
  HookOpRegistrationListener(
      const std::function<std::string(std::string)> &py_hook_function) {
...
struct HookRegisterer final {
  HookRegisterer(const std::function<std::string(std::string)> &py_function) {
    std::unique_ptr<HookOpRegistrationListener> listener =
        std::make_unique<HookOpRegistrationListener>(py_function);
