...
class PruneTool(amanda.Tool):
    ...
    def mask_forward_weight(self, context):
        ...
        if (
            "conv2d" in context["op"].__name__ and context["args"][1].shape[1] % 4 == 0
        ) or (
            "matmul" in context["op"].__name__ and len(context["args"][1].shape) == 2
        ):
            self.conv_cnt += 1
    ...
    def mask_backward_gradient(self, context):
        if (
            ...
            and "Conv" in context["bw_op"].__class__.__name__
            or "Matmul" in context["bw_op"].__class__.__name__
        ):
