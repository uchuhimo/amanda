...
def train(x, y, model, loss_fn, optim):
    # model returns the prediction and the loss that
    # encourages all experts to have equal importance and load
    ...
    print(
        "Training Results - loss: {:.2f}, aux_loss: {:.3f}".format(
            loss.item(), aux_loss.item()
        )
    )
    return model
...
def eval(x, y, model, loss_fn):
    model.eval()
    # model returns the prediction and the loss
    # that encourages all experts to have equal importance and load

    y_hat, aux_loss = model(x.float(), train=False)
    loss = loss_fn(y_hat, y)
    # total_loss = loss + aux_loss
    print(
        "Evaluation Results - loss: {:.2f}, aux_loss: {:.3f}".format(
            loss.item(), aux_loss.item()
        )
    )
