...
class Model(object):
    ...
    def __init__(
        ...
        if bottleneck:
            if version == 1:
                ...
                if gated:
                    self.block_fn = self._bottleneck_block_v2_gated
                else:
            ...
            if version == 1:
                ...
                if gated:
                    self.block_fn = self._building_block_v2_gated
                else:
        ...
        self.gate_variables: Dict[str, Any] = {}
        self.with_gates = with_gates

    def __call__(self, inputs, training, gate_variables: Dict[str, tf.Variable] = None):
    ...
    def gate_variable(self, length, name="gate", is_conv=True):
        if is_conv:
            initial = tf.constant([1.0] * length, shape=(1, length, 1, 1))
        else:
    ...
    def _bottleneck_block_v2_gated(
        self, inputs, filters, training, projection_shortcut, strides, data_format
    ):
        """
        Similar to _building_block_v2(), except using the "bottleneck" blocks
        described in:
        ...
        self.gate_count += 1
        inputs = self.gated(inputs, 4 * filters, f"gate{self.gate_count}")

        return inputs + shortcut

    def _building_block_v2_gated(
        self, inputs, filters, training, projection_shortcut, strides, data_format
    ):
        """
        Batch normalization then ReLu then convolution as described by:
        ...
        self.gate_count += 1
        inputs = self.gated(inputs, filters, f"gate{self.gate_count}")
