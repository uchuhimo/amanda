...
def test_function_hook():
    ...
    y.grad_fn.next_functions[0][0].next_functions[0][0].register_hook(lambda input, output: print(f"shape of inputs grad in hook {[i.shape for i in input]}"))
...
def test_transformer():
    class TraceTool(amanda.Tool):
        def __init__(self):
            super(TraceTool, self).__init__(namespace="amanda/pytorch")
            self.register_event(amanda.event.after_backward_op_executed, self.print_op_name)
