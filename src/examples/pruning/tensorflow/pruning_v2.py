...
class PruningTool(amanda.Tool):
    ...
    def instrumentation(self, context: amanda.OpContext):
        ...
        if (("Conv2D" == op.type and weight.shape.as_list()[3] % 4 == 0)
                or ("MatMul" == op.type and len(weight.shape) == 2)):
    ...
    def backward_instrumentation(self, context: amanda.OpContext):
        ...
        if op.type not in ["Conv2D", "MatMul"] or backward_op.type not in ["Conv2DBackpropFilter", "MatMul"]:
            return
        weight_grad = context.get_grad_outputs()[0]
        if (("Conv2D" == op.type and "Conv2DBackpropFilter" == backward_op.type and weight_grad.shape.as_list()[3] % 4 == 0)
                or ("MatMul" == op.type and "MatMul" == backward_op.type and len(weight_grad.shape) == 2)):
