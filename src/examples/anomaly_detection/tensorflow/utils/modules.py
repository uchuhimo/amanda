...
import examples.anomaly_detection.tensorflow.utils.repair as rp
import examples.anomaly_detection.tensorflow.utils.utils as utils
...
def filtered_issue(issue_list):
    if len(issue_list) > 1:
        ...
        for i in range(len(issue_list)):
            if (
                new_issue_list == []
                or problem_evaluation[issue_list[i]]
                > problem_evaluation[new_issue_list[0]]
            ):
...
def comband_solutions(solution_dic):
    ...
    while not stop:
        ...
        if solution_start == len(solution):
            stop = True
    return solution
    ...
    for strat in range(len(strategy_list)):
        for i in range(len(strategy_list[strat])):
            ...
            if solution_evaluation[sol]["modify"] > sufferance or (
                memory and solution_evaluation[sol]["memory"]
            ):
                if len(strategy_list[strat]) > 1:
                    strategy_list[strat].remove(strategy_list[strat][i])
                else:
                    print(
                        "-----WARNING, TOO STRICT FILTER! "
                        "NOW KEEPED ONLY ONE SOLUTION!-----"
                    )
...
class Repair_Module:
    def __init__(
        ...
        (
            self.gradient_vanish_strategy,
            self.gradient_explode_strategy,
            self.dying_relu_strategy,
            self.unstable_strategy,
            self.not_converge_strategy,
            self.over_fitting_strategy,
        ) = filtered(strategy_list, sufferance, memory)
            ...
            if len(self.issue_list) == 1:  # only one issue to be solved:
                ...
                if result == "solved":
                    ...
                    tmppath = os.path.join(
                        save_dir, "repair_result_single.pkl"
                    )  # save in each case monitor_tool_log
                    ...
                    print(
                        "Your model has been trained and the training problems {} have been repaired,you can find the repair log in {}, and the trained model is saved in {}.".format(
                            self.initial_issue, result_list[-1], result_list[-2]
                        )
                    )
                    return (
                        result,
                    ...
                    with open(tmppath, "wb") as f:
                        pickle.dump(tmpset, f)
                    print(
                        "Your model still has training problems {} are still exist, you can try other solutions, see the file in {}, repair log can be found in {}".format(
                            result_list[3], "/path", result_list[-2]
                        )
                    )
                    return (
                        result,
                    ...
                    if result == "no":
                        ...
                        model_name = (
                            "improved_unsolved_model_"
                            + str(self.best_potential[-3])
                            + ".h5"
                        )
                        model_path = os.path.join(tmp_dir, model_name)
                        self.best_potential[0].save(model_path)
                        print(
                            "Model has been improved but still has problem. The initial training problems in the source model is {}.\
                            The current problem is {}. You can find the repair log in {}, and the best improved model is saved \
                            in {}.".format(
                                self.initial_issue,
                                self.best_potential[3],
                                self.best_potential[-2],
                                model_path,
                            )
                        )
                        return (
                            result,
                            self.best_potential[0],
                            model_path,
                            self.best_potential[-3],
                            train_history,
                            solved_issue,
                    ...
                    if self.issue_list[0] not in solved_issue:
                        ...
                        common_log_path = os.path.join(
                            new_log_dir, "common_log_history.pkl"
                        )
                        ...
                        issue_history_path = os.path.join(
                            new_tmp_dir, "issue_history_before_repair.pkl"
                        )
                        issue_history = {}
                        ...
                        if (
                            self.best_potential == []
                            or self.best_potential[-3] < potential_list[-3]
                        ):
                            self.best_potential = potential_list.copy()
                        if (
                            "sp" in self.train_config.keys()
                        ):  # (num, model,config,issue,j,config_set):
                            self.model, self.train_config, _1, _2, _3 = notify_result(
                                self.train_config["sp"],
                                self.model,
                                self.train_config,
                                0,
                                0,
                        ...
                        model_name = (
                            "improved_model_" + str(self.best_potential[-3]) + ".h5"
                        )
                        model_path = os.path.join(tmp_dir, model_name)
                        self.best_potential[0].save(model_path)
                        print(
    ...
    def issue_repair(
        ...
        for i in range(length_solution):
            ...
            for j in range(tmp_tim):
                ...
                (
                    ...
                    tmp_sol, model, train_config, issue_type, j, config_set
                )
                if _break:
                    break  #  the solution has already been used in the source model.
                print(
                    "-------------Solution {} has been used, waiting for retrain.-----------".format(
                        tmp_sol
                    )
                )
                ...
                if (
                    tmp_sol == "estop" or tmp_sol == "ReduceLR"
                ):  # special operation for this two solution will will change the config_bk
                    config_bk["callbacks"].remove(config_bk["callbacks"][-1])
                    config["sp"] = tmp_sol

                log_file.write(
                    "{},{},{},{},{}\n".format(
                        tmp_sol, j, new_issue_list, train_result, modify_describe
                    )
                )
                log_file.flush()
            ...
            if (
                try_count >= max_try
            ):  # if tried solution is over maxtry, then go to solve the potential type.
                break
        ...
        if potential == []:
            ...
            time_used = end_time - start_time
            print(
                "------------------Not totally solved..Time used {}!-----------------".format(
                    str(time_used)
                )
            )
            log_file.write(
