...
import numpy as np
from keras.activations import elu, linear, relu, selu, sigmoid
...
def random_kwargs_list(method="gradient_clip", clipvalue=10):
    ...
    if method == "gradient_clip":
        ...
        if op_type == "clipvalue":
            kwargs_list.append(
                round(np.random.uniform(float(clipvalue - 1), clipvalue), 2)
            )
    if method == "momentum":
...
def modify_initializer(model, b_initializer=None, k_initializer=None):
    ...
    for i in range(int(last)):  # the last layer don't modify
        if "lstm" not in model.layers[i].name:
            if (kernel_initializer != None) and (
                "kernel_initializer" in model.layers[i].get_config()
            ):
            ...
            if (bias_initializer != None) and (
                "bias_initializer" in model.layers[i].get_config()
            ):
...
def not_dense_acti(model, i):
    ...
    advanced_list = [
        ...
        "thresholded_re_lu",
        "re_lu",
    ]  # ,'prelu' lead to gradient_message error
    for j in range(len(advanced_list)):
        if (i + 1) < len(model.layers) and advanced_list[j] in model.layers[
            i + 1
        ].get_config()["name"]:
...
def modify_activations(
    model, activation_name, method="normal"
):  # https://github.com/keras-team/keras/issues/9370
    """
        ...
        for i in range(int(last)):  ###考虑special情况
            ...
            for j in lstm_layer:
                if j in model.layers[i].name:
                    lstm_judge = True
            if ("activation" in model.layers[i].get_config()) and not_dense_acti(
                model, i
            ):
                if lstm_judge == True and not (
                    activation_name == "relu" or activation_name == "selu"
                ):
                    new_config = copy.deepcopy(model.layers[i].get_config())
                    new_config["activation"] = activation
                    new_layer = model.layers[i].__class__(**new_config)
                    model = replace_intermediate_layer_in_keras(model, i, new_layer)
                elif lstm_judge == True and (
                    activation_name == "relu" or activation_name == "selu"
                ):
                    new_config = copy.deepcopy(model.layers[i].get_config())
    ...
    for i in range(int(last)):  # the last layer don't modify
        ...
        if lstm_judge == False:
            if "kernel_regularizer" in model.layers[i].get_config():
                if (
                    model.layers[i].kernel_regularizer != None
                    and model.layers[i].kernel_regularizer.get_config()["l2"]
                    >= 0.7 * 0.01
                ):
                    break_count += 1
                    continue
        ...
        for j in incert_layer:
            if j in model.layers[i].get_config()["name"]:
                if model.layers[i + 1].__class__ != getattr(L, "Dropout"):
                    model = insert_intermediate_layer_in_keras(
                        model, i + 1, Dropout(rate=rate, name="Drop_{}".format(i))
                    )
                    drop_count += 1
                    i += 1
...
def DNN_skip_connect(model, layer_name="dense"):  # only activation
    ...
    for i in range(1, len(layers)):
        ...
        if (
            j % 2 != 0
            and j != 1
            and "activation" in layers[i].get_config()["name"]
            and layers[i].get_config()["activation"] == "relu"
        ):
            x = Add()([temp_x, x])
        x = layers[i](x)
    new_model = Model(input=layers[0].input, output=x)
...
def repair_strategy(method="balance"):
    ...
    return [
        gradient_vanish_strategy,
        gradient_explode_strategy,
        dying_relu_strategy,
        unstable_strategy,
        not_converge_strategy,
        over_fitting_strategy,
    ]
...
def op_gradient(model, config, issue, j, config_set):  # m
    ...
    if isinstance(config["opt"], str):
        ...
        config["opt"] = opt_cls()
    if ("clipvalue" in config["opt"].get_config()) or (
        "clipnorm" in config["opt"].get_config()
    ):
        return tmp_model, config, describe, True, config_set
    kwargs_list = random_kwargs_list(method="gradient_clip")
    config["opt"] = modify_optimizer(config["opt"], kwargs_list, method="gradient")
...
def op_relu(model, config, issue, j, config_set):  #
    tmp_model = modify_initializer(
        model, k_initializer="he_uniform", b_initializer="zeros"
    )
...
def op_tanh(model, config, issue, j, config_set):  #
    ...
    tmp_model = modify_initializer(
        tmp_model, k_initializer="he_uniform", b_initializer="zeros"
    )
...
def op_initial(model, config, issue, j, config_set):  #
    ...
    describe = "Using '{}' initializer as each layers' kernel initializer;\
         Use '{}' initializer as each layers' bias initializer.".format(
        str(init_2), str(init_1)
    )
    return tmp_model, config, describe, False, config_set
...
def op_momentum(model, config, issue, j, config_set):  # m
    ...
    describe = "Using 'momentum {}' in SGD optimizer in the optimizer.".format(
        str(kwargs_list[0])
    )
    return tmp_model, config, describe, False, config_set
...
def op_batch(model, config, issue, j, config_set):  # m
    ...
    if config["batch_size"] != batch_try:
        ...
        return model, config, describe, True, config_set
    describe = "Using 'batch_size {}' in model training.".format(
...
def op_GN(model, config, issue, j, config_set):  # m
    ...
    for i in range(min(len(model.layers), 3)):
        if ("gaussian_noise" in model.layers[i].name) or model.layers[
            i
        ].__class__ == getattr(L, "GaussianNoise"):
            return model, config, describe, True, config_set
...
def op_optimizer(model, config, issue, j, config_set):  # no
    ...
    config_set["opt_kwargs"] = {}
    describe = "Using {} optimizer in model training, the parameter setting is default.".format(
        str(tmp_opt)
    )
    return tmp_model, config, describe, False, config_set
