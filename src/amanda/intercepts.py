...
def register_builtin(func, handler):
    ...
    if func_addr not in _HANDLERS:
        ...
        _handler = get_builtin_handler(func_addr)
...
def register_function(
    ...
    if func_addr not in _HANDLERS:
        ...
        global_dict = _intercept_handler.__globals__  # type: ignore
...
def register_method_descriptor(func, handler):
    ...
    if func_addr not in _HANDLERS:
        ...
        _handler = get_method_descriptor_handler(func_addr)
        _HANDLERS[func_addr] = [func_copy, _handler]
...
def register(obj, handler):
    ...
    Usage::
        ...
        42
    """
    from intercepts.registration import (
        register_builtin,
        register_function,
        register_method,
    )
